...
result = [len(word) for word in words if word != 'the']
print(result)
...
result = [x.upper() for x in dict if dict[x] < 5000]
print(result)
...
result = [x for x in range(1000) if '4' in str(x)]
print(result)
...
the_list = [x.lower() for x in words.split() if x == 'the']
#count = 0
#count_fn = [x for x in result count += 1]
